const Borrow = {
    async showBookDetail(code) {
        ...
        let html = `<h2 style="color:#fff;">📖 ${Utils.esc(book.title)}</h2>`;
        html += `<p style="color:#fff;">编号：${Utils.esc(book.code)} | 作者：${Utils.esc(book.author)} | 分类：${Utils.esc(book.category)}</p>`;
        ...
        if (book.status === 'available') {
            ...
            html += `<div class="form-group" style="margin-top:16px;">
                <label>您的姓名</label>
                <input type="text" id="borrowName" placeholder="姓名">
                <label>借书日期</label>
            ...
            html += `<div class="form-group" style="margin-top:16px;">
                <label>您的姓名</label>
                <input type="text" id="returnName" placeholder="姓名">
                <label>归还日期</label>
                <input type="text" value="${today}" disabled>
            </div>`;
