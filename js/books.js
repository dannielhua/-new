const Books = {
    ...
    renderBookList(allBooks, searchTerm, selectedCategory) {
        ...
        if (!selectedCategory) {
            ...
            html += `<div class="book-list-header"><span class="back-btn" onclick="App.switchToBookList()">← 返回全部</span><h3 style="color:#fff;">${selectedCategory} (${filtered.length}本)</h3></div>`;
        }
        if (filtered.length === 0) html += '<p style="color:#fff;">暂无图书</p>';
        else if (selectedCategory) {
            filtered.forEach(b => html += Books.renderBookCard(b));
        } else {
            const grouped = {};
